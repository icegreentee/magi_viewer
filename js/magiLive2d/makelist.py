...
MODEL_PATH = '../../image_native/live2d_v4/'
...
for m in dl:
    ...
    ch = int(m[0:-2])
    with os.popen('findstr charaName ' + MODEL_PATH + m + '/params.json | type -d \'"\' -f 4 | sed \'s/　/ /g\'') as f:
    ...
    if not name in chars and os.path.isdir(MODEL_PATH + m[0:-2] + '00'):
        with os.popen('findstr charaName ' + MODEL_PATH + m[0:-2] + '00/params.json | type -d \'"\' -f 4 | sed \'s/　/ /g\'') as f:
